<template>
  <flowbite-themable-child tag="button" :apply="appliableTheme" class="space-x-2" :class="wrapperClasses" :disabled="disabled">
    ...
    <span :class="spanClasses">
      <div v-if="isOutlineGradient && ($slots.prefix || loadingPrefix)" class="inline-flex -mt-0.5"> <!--if outline gradient - need to place slots inside span -->
        <spinner :color="spinnerColor" :size="spinnerSize" v-if="loadingPrefix" />
        <slot name="prefix" v-else />
      </div>
      ...
      <div v-if="isOutlineGradient && ($slots.suffix || loadingSuffix)" class="inline-flex -mt-0.5"> <!--if outline gradient - need to place slots inside span -->
        <spinner :color="spinnerColor" :size="spinnerSize" v-if="loadingSuffix" />
        <slot name="suffix" v-else />
      </div>
    </span>

    <div v-if="!isOutlineGradient && ($slots.suffix || loadingSuffix)" class="inline-flex -mt-0.5"> <!--automatically add ml class if slot provided or loading -->
      <spinner :color="spinnerColor" :size="spinnerSize" v-if="loadingSuffix" />
      <slot name="suffix" v-else />
    </div>
