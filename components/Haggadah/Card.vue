...
<template>
    <Card variant="image" :img-src="imgSrc">
        <div class="flex items-start justify-between space-x-2 mb-[19px]">
            <div>
                <div v-if="isOwner" class="flex items-center space-x-1.5 mb-3.5">
                    <Badge outline>
                        <template #icon>
        ...
        <div v-if="contributorName || readTime" class="flex items-center space-x-[15px] mb-5">
            <div v-if="readTime" class="inline-flex items-center space-x-2">
                <Icon icon="icon-book-o" class="text-secondary-600" />
                <span class="text-sm text-gray-600 dark:text-gray-100 font-semibold">{{ readTime }} read</span>
            </div>

            <div v-if="contributorName" class="inline-flex items-center space-x-2">
                <img :src="contributorAvatar" alt="{{ contributorName }}" class="w-7 h-7">
                <span class="text-sm text-gray-800 dark:text-gray-100 font-semibold">
                    By {{ contributorName }}
                </span>
            </div>
        </div>
